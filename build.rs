...
fn main() {
    ...
    build_ripgrep_from_source(&out_dir, &target, &host);
}

fn build_ripgrep_from_source(out_dir: &str, target: &str, host: &str) {
    let version = "14.1.0";
    ...
    let binary_name = if target.contains("windows") { "rg.exe" } else { "rg" };
    
    let built_binary = if target != host {
        ripgrep_dir.join("target").join(target).join("release").join(binary_name)
    } else {
    ...
    fs::copy(&built_binary, &dest_binary)
        .expect("Failed to copy ripgrep binary");
    
    println!("cargo:rustc-env=RIPGREP_BINARY_PATH={}", dest_binary.display());
    println!("Ripgrep binary built successfully at: {}", dest_binary.display());
}
...
fn download_ripgrep_source(dest_dir: &Path, version: &str) {
    ...
    let status = Command::new("git")
        .args(&["clone", "--depth", "1", "--branch", version, &git_url, dest_dir.to_str().unwrap()])
        .status();
    ...
    let status = Command::new("tar")
        .args(&["-xzf", tar_path.to_str().unwrap(), "-C", parent_dir.to_str().unwrap()])
        .status()
